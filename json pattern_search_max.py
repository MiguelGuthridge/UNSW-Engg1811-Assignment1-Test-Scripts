...
This script automates testing for pattern_search_max using a JSON file containing test cases.
...
class test_case:
    ...
    def __init__(self, test_type, input_data_series, input_pattern, input_threshold, expected_output):
    ...
    def run(self, test_num):
        ...
        # Run the function
        try:
            result = psm.pattern_search_max(self.data_series, self.pattern, self.threshold)
...
print("-------------------------------------------------")
print("Pattern Search Max Tester")
...
test_file = open("json pattern_search_max.json")
...
for ctr in range(len(trials_object)):
    ...
    try:
        trials.append( test_case( case["test_type"], case["data_series"], case["pattern"], case["threshold"], case["expected_output"] ) )
    except:
